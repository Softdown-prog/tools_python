...
SOURCE_ROOT = os.path.join(TOOLS_ROOT, 'output', 'canonical_walk')
DESTINATION_ROOT = os.path.join(PROJECT_ROOT, 'assets', 'pedestrians', 'canonical_walk')

# One crop for every direction and every frame. The source is never alpha-cropped
# per pose, preserving the anchor established in the Blender calibration.
CROP_X = 64
CROP_Y = 32
CROP_WIDTH = 128
CROP_HEIGHT = 192
...
for direction in ('se', 'sw', 'nw', 'ne'):
    ...
    for index in range(4):
        ...
        cropped = common_crop(image, 'canonical_runtime_{}_{}'.format(direction, index))
...
print('CANONICAL_WALK_RUNTIME directions=4 frame_size={}x{} character_height={}'.format(TARGET_WIDTH, TARGET_HEIGHT, TARGET_CHARACTER_HEIGHT))
